...
# Rule 38: Set desired shift type before a day-off
class Rule38:
            ...
            for employee in domain.employees:
                for day in domain.days[1:]:
                    for shift1 in domain.shifts:
                        if day.date - int(timedelta(days=1).total_seconds()) <= shift1.start < day.date and employee.is_eligible(shift1) and not desired_shiftType in shift1.shift_types:
                            ...
                            slackVar = solver.add_var(lb=0, ub=1, name='desiredShiftTypeBeforeSlack_{}_{}_{}_{}'.format(employee.id, shift1.id, desired_shiftType, self.rule.rule_counter), var_type='B')
                            slackCoefficient = 0
                            varS2s = []
                            ...
                            for shift2 in shifts_on_day:
                                ...
                                if varS2:
                                    varS2s.append(varS2)
                                # c_desired_shiftype.SetCoefficient(var, 1)
                            ...
                            if not self.rule.is_mandatory:
                                ...
                                slackCoefficient = 1
                            if varS1:
                                solver.add_constr(-varS1 + xsum(varS2s) + slackCoefficient*slackVar >= 0, name='desiredShiftTypeBefore_{}_{}_{}_{}'.format(employee.id, shift1.id, desired_shiftType, self.rule.rule_counter))
                ...
                for day in domain.days[1:]:
                    for shift1 in domain.shifts:
                        if day.date - int(timedelta(days=1).total_seconds()) <= shift1.start < day.date and employee.is_eligible(shift1) and not desired_shiftType in shift1.shift_types:
                            ...
                            slack_var = solver.var_by_name('desiredShiftTypeBeforeSlack_{}_{}_{}_{}'.format(employee.id, shift1.id, desired_shiftType, self.rule.rule_counter))
