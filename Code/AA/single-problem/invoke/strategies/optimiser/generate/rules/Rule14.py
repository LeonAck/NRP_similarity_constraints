...
class Rule14:
    ...
    def set_rule(self, solver, domain):
        # minimum hours per payperiod
        for employee, payperiod_start, payperiod_end, payperiod_minutes_min, agreement_id in self.get_rule_applicable_combinations(
                domain):
            new_payperiod_minutes_min = payperiod_minutes_min - employee.previous_hours * 60
            if self.rule.parameter1 and self.rule.parameter1 > 0:
                new_payperiod_minutes_min = self.rule.parameter1
            if domain.settings.reduce_contract_hours_factor:
                new_payperiod_minutes_min = new_payperiod_minutes_min * domain.settings.reduce_contract_hours_factor
            new_payperiod_minutes_min = max(0, new_payperiod_minutes_min - employee.get_payduration_unavailabilities(
                payperiod_start.timestamp(), payperiod_end.timestamp()))
            ...
            solver.create_slacked_constraint(
                id=RULES_DEFAULTS[self.rule.tag].id(employee, payperiod_start, agreement_id, self.rule.rule_counter),
                constraint_lhs=assignment_vars,
                constraint_sense=">=",
                constraint_rhs=new_payperiod_minutes_min,
                slack_lower_bound=0,
                slack_upper_bound=new_payperiod_minutes_min,
    ...
    def add_violation_to_output(self, solver, domain, output):
        # min contract hours
        for employee, payperiod_start, payperiod_end, _, agreement_id in self.get_rule_applicable_combinations(
                domain):
            contract_hours_slack_var = solver.find_slack_variable_constraint(
                RULES_DEFAULTS.min_contract_hours.id(employee, payperiod_start, agreement_id, self.rule.rule_counter))
    ...
    def get_rule_applicable_combinations(self, domain):
        for employee in domain.employees.get_rule_applicable_employees(self.rule):
            payperiod_length = 7
            if (employee.payperiod_minutes_min > 0 or len(employee.agreements) > 0) and self.rule.is_applicable(None,
                                                                                                                employee):
                if len(employee.agreements) > 0:
                    for agreement in employee.agreements:
                        ...
                        agreement_end = end_of_day(
                            parse_timestamp(agreement.end if agreement.end else domain.settings.end,
                                            domain.settings.time_zone))
                        if agreement.payperiod_length:
                        ...
                        while payperiod_start < agreement_end:
                            payperiod_end = min(agreement_end,
                                                get_payperiod_end(employee, payperiod_start, payperiod_length,
                                                                  domain.settings.time_zone))
                            if len(domain.days.get_days_in_period(payperiod_start, payperiod_end)) > 0:
                                min_minutes = adjust_minutes(domain,
                                                             agreement.payperiod_minutes_min,
                                                             payperiod_start, payperiod_end, payperiod_length, agreement.scale_contract_minutes)

                                yield employee, payperiod_start, payperiod_end, min_minutes, agreement.id
                            payperiod_start = payperiod_end
                else:
                    if employee.payperiod_length:
                        payperiod_length = employee.payperiod_length
                    payperiod_start = start_of_day(
                        parse_timestamp(employee.payperiod_start if employee.payperiod_start else domain.settings.start,
                                        domain.settings.time_zone))
                    ...
                    while payperiod_start < settings_end_dt:
                        payperiod_end = get_payperiod_end(employee, payperiod_start, payperiod_length,
                                                          domain.settings.time_zone)
                        if len(domain.days.get_days_in_period(payperiod_start, payperiod_end)) > 0:
                            min_minutes = adjust_minutes(domain,
                                                         employee.payperiod_minutes_min,
                                                         payperiod_start, payperiod_end, payperiod_length)

                            yield employee, payperiod_start, payperiod_end, min_minutes, "no_agreement"
                        payperiod_start = payperiod_end
