...
# Rule 68: Penalty for assigning more than the maximum number of shifts per shift group
class Rule68:
    def __init__(self, rule):
        self.rule = rule

    def set_rule(self, solver, domain):
        max_on_groups = self.rule.parameter1
        for group_id in self.get_rule_applicable_combinations(domain):
            ...
            solver.create_slacked_constraint(
                ...
                constraint_sense="<=",
                constraint_rhs=max_on_groups,
                slack_lower_bound=0,
                slack_upper_bound=0 if self.rule.is_mandatory else max_on_groups,
