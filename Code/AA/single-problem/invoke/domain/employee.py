...
class Employee:
    ...
    def is_eligible(self, shift):
        ...
        if not shift.is_fixed:
            ...
            overlappingFixedShifts = [fixed_shift for fixed_shift in self.fixed_shifts
                                      if (fixed_shift.start < shift.end
                                          and fixed_shift.end > shift.start)]
    ...
    def check_shift_inside_availablities(self, shift):
        if self.use_availabilities:
            if not any(availability.start <= shift.start
                       and shift.end <= availability.end for availability in
                       self.availabilities):
                return False
        return True

    def check_shift_outside_unavailabilities(self, shift):
        if any(shift.end > unavailability.start and shift.start < unavailability.end
               for unavailability in self.unavailabilities):
            return False
        return True
